...
config.singleBandDeblend.catchFailures=False

# Mask planes to ignore when performing statistics
config.singleBandDeblend.maskPlanes=['SAT', 'INTRP', 'NO_DATA']
...
config.singleBandDeblend.maskLimits={}
...
config.singleBandDeblend.weightTemplates=False

# Try to remove similar templates?
config.singleBandDeblend.removeDegenerateTemplates=False
...
config.singleBandDeblend.maxTempDotProd=0.5

# Apply a smoothing filter to all of the template images
config.singleBandDeblend.medianSmoothTemplate=True

# Maximum number of iterations to deblend a single parent
config.multiBandDeblend.maxIter=200

# Relative error to use when determining stopping criteria
config.multiBandDeblend.relativeError=0.01

# Number of iterations between recentering
config.multiBandDeblend.recenterPeriod=5
...
config.multiBandDeblend.exactLipschitz=True

# Make models compact and sparse
config.multiBandDeblend.sparse=True

# Make models monotonic
config.multiBandDeblend.monotonic=True

# Make models symmetric
config.multiBandDeblend.symmetric=True
...
config.multiBandDeblend.processSingles=False

# Whether or not to store the history for each source
config.multiBandDeblend.storeHistory=False

# Whether or not to process isolated sources in the deblender
config.multiBandDeblend.badMask=['BAD', 'CR', 'NO_DATA', 'SAT', 'SUSPECT']
...
# Only deblend the brightest maxNumberOfPeaks peaks in the parent (<= 0: unlimited)
config.multiBandDeblend.maxNumberOfPeaks=0

# Maximum area for footprints before they are ignored as large; non-positive means no threshold applied
config.multiBandDeblend.maxFootprintArea=1000000

# Maximum linear dimension for footprints before they are ignored as large; non-positive means no threshold applied
config.multiBandDeblend.maxFootprintSize=0

# Minimum axis ratio for footprints before they are ignored as large; non-positive means no threshold applied
config.multiBandDeblend.minFootprintAxisRatio=0.0

# Mask name for footprints not deblended, or None
config.multiBandDeblend.notDeblendedMask='NOT_DEBLENDED'

# If True, catch exceptions thrown by the deblender, log them, and set a flag on the parent, instead of letting them propagate up
config.multiBandDeblend.catchFailures=False

# Guarantee that all peaks produce a child source.
config.multiBandDeblend.propagateAllPeaks=False
